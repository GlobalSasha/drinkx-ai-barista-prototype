...
const drinkGrid = document.querySelector("#folio-drinks");
const filterButtons = document.querySelectorAll(".folio-order-filter");
const settings = document.querySelector("#folio-settings");
const configName = document.querySelector("#folio-config-name");
const configPrice = document.querySelector("#folio-config-price");
const addCustom = document.querySelector("#folio-add-custom");
const addLabel = document.querySelector("#folio-add-label");
const cartCount = document.querySelector("#folio-cart-count");
const cartTotal = document.querySelector("#folio-cart-total");
const cartSummary = document.querySelector("#folio-cart-summary");
const resetOrder = document.querySelector("#folio-reset-order");
...
function fitFolioToViewport() {
  ...
  document.documentElement.style.setProperty("--folio-scale", scale.toFixed(5));
  document.documentElement.style.setProperty("--folio-fit-width", `${(DESIGN_WIDTH * scale).toFixed(2)}px`);
  document.documentElement.style.setProperty("--folio-fit-height", `${(DESIGN_HEIGHT * scale).toFixed(2)}px`);
}
...
function renderDrinks() {
  drinkGrid.innerHTML = visibleDrinks().map((drink) => `
    <article class="folio-order-card${drink.id === state.selectedId ? " is-selected" : ""}">
      <button class="folio-order-quick" type="button" data-quick-add="${drink.id}" aria-label="Быстро добавить ${drink.name} за ${price(drink.price)}">
        <span class="folio-order-index">${String(drinks.indexOf(drink) + 1).padStart(2, "0")}</span>
        <span class="folio-order-copy"><strong>${drink.name}</strong><small>${drink.volume} / STANDARD</small></span>
        <b>+${drink.price}</b>
      </button>
      <button class="folio-order-customize" type="button" data-customize="${drink.id}">CUSTOMIZE_RECIPE →</button>
    </article>
  `).join("");
}
...
function renderSettings() {
  settings.innerHTML = Object.entries(optionLabels).map(([key, label]) => `
    <label class="folio-order-setting">
      <small>${label}</small>
...
function renderOrder() {
  ...
  cartSummary.textContent = state.lastOrder
    ? `LAST: ${state.lastOrder.name} / ${state.lastOrder.volume} / ${state.lastOrder.milk} / ${state.lastOrder.syrup} / ${state.lastOrder.sugar}`
    : "ВЫБЕРИТЕ НАПИТОК";
}
...
function addSelectedDrink(isCustom = false) {
  ...
  renderOrder();
  showToast(isCustom ? `${drink.name} / CUSTOM ADDED` : `${drink.name} / STANDARD ADDED`);
}
...
drinkGrid.addEventListener("click", (event) => {
  ...
  if (customize) {
    ...
    showToast("ИЗМЕНИТЕ РЕЦЕПТ И ДОБАВЬТЕ ВАРИАНТ");
  }
});
